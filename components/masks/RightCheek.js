...
export const RightCheekMask = ({ algo_tech_name, imageData }) => {
    ...
    useEffect(() => {
        ...
        if(maskMain?.result?.masks_restored?.length){
            ...
            const maskMainRestored = maskMain.result.masks_restored.filter(el=>{
        		...
        		return el.tech_name === "right_cheek"
    ...
    return (
        <div>
            ...
            <svg preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg" ref={svgHolder}>
                ...
                <clipPath id="rightCheekMaskPolygons" ref={polygonsHolder} />
                {/* img to clip */}
                <image id="image" width="812" height="812" clipPath="url(#rightCheekMaskPolygons)" ref={imageHolder} />
                <g id="subPolygons" clipPath="url(#rightCheekMaskPolygons)" ref={groupHolder} />
            </svg>
        </div>
    );
