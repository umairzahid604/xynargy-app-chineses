...
export const ForeheadMask = ({ algo_tech_name, imageData }) => {
    ...
    useEffect(() => {
        ...
        if(maskMain?.result?.masks_restored?.length){
            ...
            const maskMainRestored = maskMain.result.masks_restored.filter(el=>{
                ...
                // return el.tech_name !== 'left_eye_area' && el.tech_name !== 'right_eye_area' && el.tech_name !== 'nose' && el.tech_name !== 'right_cheek' && el.tech_name !== 'left_cheek' && el.tech_name !== 'chin'
        		return el.tech_name === "forehead"
    ...
    return (
        <div>
            ...
            <svg preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg" ref={svgHolder}>
                ...
                <g id="subPolygons" clipPath="url(#maskPolygons)" ref={groupHolder} />
            </svg>
        </div>
    );
